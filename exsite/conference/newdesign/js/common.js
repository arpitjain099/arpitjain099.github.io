...
function check() 
{
	...
	{
		alert("Plese select your counrty");
	...
	*/
	/*else if(chkGraduationYear == true) 
	{
		...
		{
			alert("Please select your campus name");
	...
	else if(mode_of_payment_chosen == 'student_only')
	{
		...
		{
			alert("Please select other way of payment");
			valid=false;
		}
	}
	/* end */
...
if(len!=0)
{
	...
	var count=0;
	 ...
	 if(count>3)
	 {
	 	alert(' - cannot be more than Three times');
	 	return false;
	 }
	...
	var c=0;
	 for (i = 0; i < strString.length; i++)
	 {
	  ...
	  {
		alert('Please do not enter all 0 ');
	  	return false;
	  }
	...
	for (i = 0; i < strString.length ; i++)
	{
		 strChar = strString.charAt(i);
	  	...
	  	if(strChar=='-')
	  	{
	  		if(flag==1)
	  		{
	  			alert('Please do not enter consecutive -');
	  			return false;
	  		}
	  		else
	  		{
   ...
   //  test strString consists of valid characters listed above
	 for (i = 0; i < strString.length && blnResult == true; i++)
	 {
	  ...
	  if (strValidChars.indexOf(strChar) == -1)
		 {
		 	alert('Please enter nuemric values in Phone field');
		 	blnResult = false;
		 }
	  }
}
